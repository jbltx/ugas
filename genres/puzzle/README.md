...
> match-scoring pipeline that multiplies a base match through a combo channel and a chain channel.
...
channels, move economy, and cascade chain generalize across the wider puzzle family.
...
| `entities/effect_combo_build.yaml` | `HasDuration` `+1` `ComboMultiplier`, refreshing its window each match (`Combo` channel) |
| `entities/effect_cascade_chain.yaml` | Instant `+1` `ChainLength` per cascade depth (`Chain` channel) |
...
| `entities/input_actions.yaml` | `SelectTile`, `Drag`, `ActivatePowerup`, `UseHint` actions |
...
| `entities/input_modifiers.yaml` | `SwipeDeadzone`, `MouseDragSensitivity` input modifiers |
